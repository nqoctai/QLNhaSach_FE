...
import {
  callFetchAllCategoryAndQuantityBookAreSold,
  callFetchDashboard,
  callFetchGenderRatio,
  callFetchRevenue,
  callFetchTop5BookAreSold,
  callFetchView6,
} from "../../services/api";
import {
  Chart as ChartJS,
...
const AdminPage = () => {
  ...
  const fetchCategoryData = async () => {
    ...
    if (res && res.data) {
      const totalQuantity = res.data.reduce(
        (total, item) => total + item.soLuongBan,
        0
      );
      setBarCategoryData({
        ...
        datasets: [
          {
            label: "Tỷ lệ bán",
            data: res.data.map(
              (item) => (item.soLuongBan / totalQuantity) * 100
            ),
            backgroundColor: [
              "rgba(255, 99, 132, 0.2)",
              "rgba(54, 162, 235, 0.2)",
  ...
  const fetchTop5BookSold = async () => {
    ...
    if (topBooksData && topBooksData.data) {
      const res = {
        labels: topBooksData.data.map((item) => item.bookName),
        datasets: [
          {
            label: "Số lượng bán",
            data: topBooksData.data.map((item) => item.totalQuantity),
      ...
      <div style={{ height: "100vh", overflowY: "auto", padding: "20px" }}>
        <Row gutter={[40, 40]}>
          ...
          <Col span={6}>
            <Card title="" bordered={false}>
              <Statistic
                title="Số lượng đơn hàng thành công"
                value={dataDashboard.tongSoLuongDatHangThanhCong}
                precision={2}
                formatter={formatter}
              />
            </Card>
          ...
          <Col span={6}>
            <Card title="" bordered={false}>
              <Statistic
                ...
                value={dataDashboard.tongSoLuongNhanVien}
                precision={2}
                formatter={formatter}
              />
            </Card>
        ...
        <Row gutter={[40, 40]} style={{ marginTop: "20px" }}>
          <Col span={12}>
            <Card
              ...
              title="Biểu đồ số lượng bán theo thể loại"
              bordered={false}
            >
        ...
        <Row gutter={[40, 40]} style={{ marginTop: "20px" }}>
          ...
          <Col span={12}>
            <Card
              style={{ height: "500px", overflowY: "scroll" }}
              title="Top 5 sách bán chạy nhất"
              bordered={false}
            >
